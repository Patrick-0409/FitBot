...
class SchedulesService {
  ...
  Future setNotifSchedules() async {
    ...
    await FirebaseFirestore.instance
            .collection('schedules')
            .where('uid', isEqualTo: user?.uid)
            .where('startDate', isEqualTo: DateFormat.yMd().format(DateTime.now()))
                  ...
                  if (querySnapshot.docs.length>0) {
                      for (var i = 0; i < querySnapshot.docs.length; i++) {
                        if(querySnapshot.docs[i]['remind']>0){
                          ...
                          DateTime date = DateFormat.yMd().add_Hm().parse(querySnapshot.docs[i]['startDate'] + " " + newTime);
                        ...
                        NotificationService.showScheduledNotification(
                          ...
                          payload: 'schedule',
                          scheduledDate: DateFormat.yMd().add_Hm().parse(querySnapshot.docs[i]['startDate'] + " " + querySnapshot.docs[i]['startTime']),
                        );
                      }
                  }
              });
